...
**Before increment A begins, spike S8 (next in [the research plan](../research/0-research-plan.md)'s arc) assembles all campaign assets into one working end-to-end prototype — ingest → diagnose → explain → signals → TUI — validating this split's seams (especially schema/ingest/replay boundaries and the TUI's API-only rule) before any packaging hardens.**
