...
> **Post-research corrections (2026-08-01, from [stage 8-support](./8-support-engine-introspection-and-ebpf.md)):** the independent engine survey confirms the architecture but corrects the tier map in four places, superseding the text below wherever they differ. (1) **SGLang, not vLLM, is the first target**: SGLang ships an official `--forward-hooks` startup option (glob-matched `named_modules()` + hook factory, attached via `register_forward_hook` in `ModelRunner.initialize()`) — the only reviewed server with a documented load-time activation contract; the open problem is request identity in the hook, not tensor access. (2) **vLLM monitor mode currently *requires a patch***: its general plugin system does not hand a plugin the loaded model or the batch-row→request map; the "Observation Plugin" RFC (March 2026) that would provide exactly this remains unlanded; the v0.18 hidden-state extractor is a deep-mode primitive (full vectors to safetensors via KV-connector), not a probe hook. (3) **The TensorRT-LLM claim is obsolete for current releases**: 1.2 removed the compiled-engine backend entirely — TensorRT-LLM is now PyTorch-only and *supports* monitor mode out-of-tree via request-aware logits processors plus `additional_model_outputs`. (4) **llama.cpp's `cb_eval` graph callback is public and sees *named* ggml tensors** (a better primitive than assumed) but stock `llama-server` doesn't expose it (custom libllama host or server patch needed), and tensors are *not* reliably in host RAM (Metal/CUDA/Vulkan backends; host reads may force sync copies). Ollama exposes neither. The eBPF verdict is confirmed with nuance: bpftime can inject PTX probes and read registers/memory, so semantic activation recovery is *possible* — but only as a reverse-engineered, version-pinned patch requiring engine/kernel-specific layout knowledge; spike S5 explores exactly how far that ceiling can be pushed.
>
