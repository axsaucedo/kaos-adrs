...
- **Operator responsibilities.** The operator reconciles `MemoryStore` resources: provisioning or binding the required datastores, wiring credentials as secrets (not operator-wide defaults), scheduling consolidation/forgetting jobs, exposing health and metrics, and validating model references. The Agent CRD keeps a slim memory block that selects a store and sets per-agent scope, context budget, and tier toggles.
...
- **Polyglot, bound by the CRD.** The target supports a vector store (pgvector/Qdrant/RedisVL), an optional graph store (for the relational/temporal tier), a relational store (profiles, provenance, metadata), and Redis (working memory, queues, caches) — chosen per `MemoryStore` and reconciled by the operator. KAOS's existing Redis is reused wherever possible to minimize infrastructure delta ([KAOS-R5-5](./KAOS-R5-5-redis-agent-memory-server.md)).
