...
- **Write discipline.** Lightweight writes (append short-term memory) are inline; expensive writes (extraction, embedding, graph construction, consolidation) run as background jobs, so KAOS's autonomous loops — which emit many events ([KAOS-R1](./KAOS-R1-memory-features-and-limitations.md)) — never pay extraction latency on the hot path.
...
- **Two-layer interface.** The `Memory` ABC gains a long-term sub-interface (write/extract, search/recall, consolidate, forget) alongside its existing short-term operations, so a backend implements one or both layers. The Redis AMS two-tier model ([KAOS-R5-5](./KAOS-R5-5-redis-agent-memory-server.md)) shows this is natural.
- **Pluggable long-term backends.** Concrete backends — a Mem0 backend, a Redis AMS backend, a future Graphiti backend, and the existing Redis/local short-term backends — sit behind the interface, selected by the `MemoryStore` the agent references. The bridge fidelity issues in [KAOS-R1](./KAOS-R1-memory-features-and-limitations.md) (tool calls not replayed; hard truncation) are fixed in the short-term layer regardless of long-term engine.
- **Recall and write hooks.** Before a run, the runtime hydrates context from short-term memory plus a scope-filtered long-term recall; after a run, it appends short-term memory inline and enqueues long-term extraction/consolidation as background work. Recall results are injected as a structured memory block and/or as tools, per agent configuration.
...
## Deployment topology: the long-term engine as a central service

For a multi-tenant KAOS fleet, the long-term engine is run as a **central shared service**, not embedded as a library inside every agent pod. The primary engine ([KAOS-R6](./KAOS-R6-tool-selection.md): Mem0) is a tier-1 dependency on the agent hot path, so its topology must make recall fast and the engine highly available while keeping its expensive work off the agent turn.

- **Central service over embedded library (the default).** A dedicated memory-service Deployment (the engine — e.g. Mem0 — behind a ClusterIP) is referenced by agents as a thin HTTP client behind the `Memory` ABC. This keeps the engine's heavy dependencies out of every agent image, centralizes connection pooling, versioning, and observability, isolates extraction load from the agent runtime, enables shared memory across A2A-delegating fleets, and gives a single surface to scale, secure, and upgrade. The embedded-library mode remains supported as an explicit non-default for single-tenant, low-volume, or edge/air-gapped deployments where a network hop and a shared service are unwanted.
- **Split the path, not the dependency.** Recall is the only memory operation on the agent's critical path: it is **synchronous, low-latency, no-LLM-on-read, and horizontally scalable**, served from the external vector store. Writes and extraction — which invoke an LLM and are expensive ([KAOS-R5-1](./KAOS-R5-1-mem0.md)) — are **enqueued** (on the Redis queue) and processed by separate background workers, never blocking the turn. This is how a tier-1 hot-path dependency stays consistent with the "background by default" write discipline above.
- **Statefulness must be engineered away for HA.** Promoting the engine to a replicated tier-1 service requires resolving its server-mode statefulness. [KAOS-R5-1](./KAOS-R5-1-mem0.md) found the Mem0 server holds a singleton in-process instance and a **per-container SQLite history database**, which is unsafe under naive replication. The target therefore makes the **read path stateless** behind the external, HA vector store (the scaling tier), and moves history/metadata off SQLite onto the shared, separately-managed relational store (or isolates a single writer). This is the concrete engineering cost of making Mem0 tier-1, and it must be addressed at the integration layer rather than assumed.
- **Resilience and blast radius.** As a shared tier-1 service, an outage degrades long-term recall fleet-wide, so the design treats memory as **augmentation, not correctness**: a recall timeout or failure lets the agent proceed on short-term memory alone, behind a tight client-side timeout, a circuit breaker, and an optional recall cache. The service runs with HA replicas and PodDisruptionBudgets, enforces per-tenant rate limits and quotas, and shields the LLM extraction path from rate-limit storms (bounded worker concurrency, backpressure on the queue).
- **Database managed separately.** Consistent with the datastore-lifecycle scope above, the central service **binds to** externally-operated, HA vector and relational stores via connection/secret references; KAOS operates the memory-service workload and its workers, not the databases.
- **Operator and CRD.** The operator deploys and manages the memory-service Deployment, the extraction workers, and the queue binding. The `MemoryStore` CRD therefore carries a topology aspect — service-versus-embedded mode, replica count and autoscaling targets, timeout/circuit-breaker policy, and per-tenant rate/quota limits — alongside its storage bindings and policy fields.


## Storage and distribution

- **Polyglot, bound by the CRD.** The target supports a vector store (pgvector/Qdrant/RedisVL), an optional graph store (for the relational/temporal tier), a relational store (profiles, provenance, metadata), and Redis (short-term memory, queues, caches) — selected per `MemoryStore` and **bound to** by the operator via connection/secret references rather than provisioned by it (see control-plane datastore-lifecycle scope above). KAOS's existing Redis is reused wherever possible to minimize infrastructure delta ([KAOS-R5-5](./KAOS-R5-5-redis-agent-memory-server.md)).
- **Durable, distributed, consistent.** Long-term memory is persistent and shared across replicas (as RedisMemory already is for short-term memory in [KAOS-R1](./KAOS-R1-memory-features-and-limitations.md)), with the concurrency hazards noted there (non-atomic get-or-create) fixed via atomic primitives. Background jobs scale horizontally on a Redis-backed queue.
...
- **Keep short-term memory.** The existing local/Redis conversation log remains the short-term tier; fix its fidelity (replay tool calls, summarize instead of truncate) and concurrency hazards.
