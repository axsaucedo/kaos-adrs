...
- **Resilience and blast radius.** As a shared tier-1 service, an outage degrades long-term recall fleet-wide, so the design treats memory as **augmentation, not correctness**: a recall timeout or failure lets the agent proceed on working memory alone, behind a tight client-side timeout, a circuit breaker, and an optional recall cache. The service runs with HA replicas and PodDisruptionBudgets, enforces per-tenant rate limits and quotas, and shields the LLM extraction path from rate-limit storms (bounded worker concurrency, backpressure on the queue).
