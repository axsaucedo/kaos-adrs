...
## Recalibration from the shipped service

The central service landed with the surface below; this plan is adjusted to consume exactly what shipped rather than an assumed shape.

- **The live HTTP surface is three POST routes plus two probes.** `POST /v1/recall` → `RecallResponse {facts: [native Mem0 dicts], working: {summary, recent: [(role, text)]}, block, degraded}`; `POST /v1/write` → `202` with `WriteResponse {accepted, scheduled, degraded}`; `POST /v1/forget` → `ForgetResponse {forgotten, degraded}`; `GET /healthz` (liveness) and `GET /readyz` (store readiness). There is **no standalone working-read endpoint** — the short-term tier is returned *inside* the recall response.
- **One recall call serves both long-term and message-history.** Because `recall` returns `working.summary` + `working.recent` alongside long-term `facts` and a ready-to-inject `block`, the runtime assembles message-history from the same call it uses for long-term recall — it does not need a separate short-term tier fetch. The bridge (TODO 4) reads `working.recent`/`working.summary` off the recall response.
- **`block` is already assembled server-side.** The service returns a deterministic injection `block`; the runtime injects it verbatim (TODO 5) and only falls back to hand-assembling from `facts`/`short_term` if it needs a different presentation.
- **Best-effort is the contract, not an add-on.** Every response carries `degraded`; recall returns 200 + `degraded=true` (short-term-only) on long-term failure, write returns 202 + `degraded=true` on a swallowed scheduling error under fail-soft. `ServiceMemory` must treat `degraded` as normal and never raise on it; only transport/timeout errors are caught and converted to a degraded local result.
- **Write is fire-and-confirm (202).** The runtime calls `write` after a turn and returns immediately; `scheduled=true` means long-term extraction is queued in the service's background runner. The runtime must not wait on extraction and must keep `write` off the hot path (call after the response is produced, and in the autonomous loop without blocking).
- **Scope owner presence is non-optional, including `shared`.** Reconfirmed at the service boundary: a scope that resolves to no Mem0 owner id makes the service raise. `scope_from_deps` (TODO 3) must always populate the field the `level` requires and the reserved shared-owner id for `shared`.
- **`failure_mode` defaults to `soft` on the wire.** `ServiceMemory` sends `soft` unless the Agent config selects `strict`; the per-request policy fields the service accepts are `top_k`, `include_short_term`, `short_term_token_budget` (recall) and `infer`, `failure_mode` (write).
- **Trace propagation, not span nesting by reference.** The service emits its own `kaos.memory.*` spans under a once-per-process tracer; the runtime should propagate context over HTTP (inject trace headers) so the service spans nest under the runtime span, rather than expecting to share a tracer object.

## Problem statement

Make the agent runtime consume the central memory service through a redesigned, tiered `Memory` contract. Under the alpha clean-break stance ([adr_0003](../adrs/adr_0003_memory-interface-and-runtime-data-plane.md)) the short-term-only event-deque `Memory` and the `MEMORY_*` configuration are replaced: the runtime gains a **service-client backend** that calls M2 for recall (synchronous, hot path) and write/forget; the message-history bridge is fixed to replay **full-fidelity** history including tool calls, tool returns, and delegation parts; recalled long-term context is injected as a **structured memory block** by default with **opt-in save/load memory tools**; and tenant/agent **scope is derived and injected server-side** from the authenticated request context and the agent's verifiable identity (`AgentDeps`), never from caller- or model-supplied arguments. Hooks wire into the agent run, the autonomous loop, and A2A delegation.
...
- `pais/tools.py:55-108` `DelegationToolset` passes `ctx.deps.memory` + `session_id` into `execute_delegation`; `:130-144` loads `get_session_events` and forwards recent user/assistant context to sub-agents — the A2A short-term-memory hand-off point.
...
- **`Memory` ABC (tiered).** Keep the short-term tier methods the runtime needs for message-history assembly, and add `recall(scope, query, …)`, `write(scope, events)`, `forget(scope)`. A short-term-only/null backend leaves long-term ops no-op. The contract is shaped around M2's HTTP surface, not an abstract algebra ([adr_0002](../adrs/adr_0002_memory-implementation-mem0-and-pydantic-ai-integration.md)).
- **`ServiceMemory(Memory)`.** An `httpx`-backed client of M2: `recall`→`POST /v1/recall`, `write`→`POST /v1/write`, `forget`→`POST /v1/forget`; short-term tier reads for message-history come from the service (the short-term tier lives in the service per [adr_0004](../adrs/adr_0004_deployment-topology-and-control-plane.md), so the runtime no longer holds an in-process deque). Fail-soft: a recall error yields short-term-only/empty context and a degraded span, never an exception that aborts the turn.
...
1. **Tiered `Memory` ABC + Null backend.** Redesign the ABC in `pais/memory.py`: short-term tier methods used by the bridge + `recall`/`write`/`forget`; remove the in-process `deque` ownership from the contract; keep `NullMemory` no-op for the disabled path. Delete `LocalMemory`/`RedisMemory` (alpha clean break) or reduce `Local` to a thin test double. **Validate**: unit tests for the ABC shape and Null no-op; `cd pydantic-ai-server && pytest tests/ -k memory -v && make lint`.

2. **`ServiceMemory` client.** Implement the `httpx` client of M2 with fail-soft recall, `write`, `forget`, and short-term tier reads; configurable endpoint/timeout; per-request policy (budget, summary, presentation, failureMode). **Validate**: unit tests against a stub M2 server (or `respx`) — recall/write/forget call the right routes, a recall error degrades to short-term-only, timeouts fail soft; `pytest tests/test_service_memory.py -v`.
...
6. **Wire run + autonomous loop + delegation; replace config.** Replace `_create_memory`/`MEMORY_*` with the new env contract and construct `ServiceMemory`; call `recall` before the run and `write` after (and within the autonomous loop in `pais/a2a.py`); update `DelegationToolset` (`pais/tools.py`) so the A2A hand-off uses the service short-term tier and carries the delegated scope prefix context (the inheritance policy itself is M5). **Validate**: unit/integration tests — a run recalls then writes via the service, the autonomous loop writes without blocking, delegation forwards short-term context from the service; `pytest tests/ -v && make lint`.
