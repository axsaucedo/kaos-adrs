...
- **`failure_mode` defaults to `soft` on the wire.** `ServiceMemory` sends `soft` unless the Agent config selects `strict`; the per-request policy fields the service accepts are `top_k`, `include_working`, `working_token_budget` (recall) and `infer`, `failure_mode` (write).
