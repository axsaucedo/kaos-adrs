...
- Storage modes are fixed by [adr_0004](../adrs/adr_0004_deployment-topology-and-control-plane.md): `local` = embedded Chroma + SQLite short-term table on one PVC, single replica; `external` = pgvector + plain short-term table on shared Postgres, stateless `replicas: 2+`.
...
All production code: the long-term adapter and short-term store as real modules (M1), the service (M2), the runtime client (M3), the CRD/operator (M4), scope enforcement and erasure (M5), the installer (M6), and HA/durable-queue/e2e/docs (M7). M0 only proves the pieces compose and records what the real Mem0 contract looks like.
