...
5. **Promote M0 harnesses to committed e2e.** Add `operator/tests/e2e/test_memory_*_e2e.py` covering scope-correct recall on both providers, model routing through a `ModelAPI`, short-term tier summarization, and deploy-and-restart durability; reuse `conftest.py`. **Validate**: e2e green on local KIND and via `workflow_dispatch`; suppress to `./tmp/null`.
