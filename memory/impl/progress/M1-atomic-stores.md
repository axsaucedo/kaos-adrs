...
2. **Config + scope value objects** — `config.py` (typed `StorageConfig`/`LocalStorage`/`ExternalStorage`/`ModelConfig`/`ShortTermTierConfig` with a `.resolved()` accessor) and `scope.py` (`Scope`, `ScopeLevel`, the reserved shared-owner key, and the Mem0 owner mapping). 15 tests.
...
4. **Short-term tier store** — `shortterm.py` + `tokens.py`: a scope-keyed relational buffer with a token budget and rolling summary, eviction-by-summarization (never truncation), over a `_Backend` abstraction that hides SQLite vs Postgres. 8 tests (SQLite + Postgres).
5. **Model client** — `models.py`: an OpenAI-compatible chat-completions binding for short-term tier summarization, driven by the same `ModelConfig` as Mem0's extraction/embedding, with an `as_summarizer` adapter for the short-term store. 3 tests via a mocked transport.
