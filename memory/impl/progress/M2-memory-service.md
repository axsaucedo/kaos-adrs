...
1. **ASGI app + probes** — `service.py` adds the `MemoryService` composition object (long-term store, working store, scheduler) and a `create_app` FastAPI factory exposing `/healthz` (liveness, store-independent) and `/readyz` (readiness, which pings the stores, not the model endpoint, so a service starts and reports not-ready before its `ModelAPI` is reachable). `settings.py` builds the service from `KAOS_MEMORY_*` env, and `__main__.py` runs it under uvicorn — the same stack as the agent runtime.
2. **Synchronous recall** — `POST /v1/recall` assembles long-term facts (passed through with Mem0's native fields) and the working-tier slice (rolling summary plus recent turns) into a deterministic injection block via `presentation.assemble_block`. Recall is best-effort: a long-term failure degrades to working-only with `degraded=true` rather than erroring. The SQLite working store was made threadpool-safe (`check_same_thread=False` plus an `RLock` around its read-modify-write paths) because FastAPI runs sync handlers on a threadpool.
3. **Write + scheduled extraction** — `POST /v1/write` appends the turn to the working tier synchronously (durable, in-band) and schedules long-term fact extraction off the request path, returning `202` with `scheduled`/`degraded` flags. Fail-soft swallows scheduling errors and reports degraded; strict surfaces them.
...
5. **Forget + rolling summary** — `POST /v1/forget` clears a scope's working tier and deletes its long-term memories (fail-soft still clears the working tier and reports degraded). Server-side rolling summary is verified end to end: overflow folds into the summary while recent turns stay verbatim.
...
- Container built and run in local mode: `/healthz` and `/readyz` return 200, `POST /v1/write` (infer=false) returns 202 and the turn surfaces in `/v1/recall` working context and the assembled block.
