...
## A useful memory example must show an agent using the store, not raw service calls

The first cut of the worked example only POSTed to the memory service directly, which demonstrated the service but never how an agent actually uses memory — the thing a reader needs. The reworked example deploys a memory-enabled Agent and sends it ordinary chat requests, then validates through the memory service recall API that the runtime persisted the conversation. Three facts make this work model-independently: (1) the runtime applies an automatic baseline around every run — recall-and-inject before, flush-and-persist after — so a bound agent writes to the store with no tool call and no code; (2) the short-term append is synchronous and verbatim, so it lands even when the model is mocked and no embedder exists (only long-term extraction degrades); and (3) `SHARED` scope always resolves to a reserved owner, so it needs no authenticated principal — making it the right scope for a simple cross-session demo, where `USER` would require request-carried identity and `SESSION` would defeat cross-session recall. The mock model is scripted via `DEBUG_MOCK_RESPONSES`, whose queue is reset per external request, so each `kaos agent invoke` re-consumes the list from the start and both sessions get a clean deterministic reply. The explicit `save_memory`/`search_memory` tools (`tools: all`) are shown in the config and explained, but their genuine semantic effect is deferred to the pgvector section because they write long-term and need a real embedder.

## Keep cluster orchestration in bash, not Python subprocess

An example that shells out to `kubectl` through Python `subprocess`/`Popen` (a jsonpath helper, a polling loop, a persisted port-forward) reads as overcomplicated. Doing apply/wait/port-forward directly in bash cells is clearer, and the cross-cell port-forward-persistence problem that pushed the original toward `Popen` disappears when each interaction is a self-contained bash cell that port-forwards, curls, and tears the forward down in one shell; a tiny temp JSON file bridges the recalled data into a Python assert cell.
