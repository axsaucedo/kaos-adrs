...
The active short-term window is computed on **read**, not maintained on every write. This lazy-read strategy keeps writes append-only and avoids per-write overflow scans. Folding is triggered by a **high-water** threshold and evicts toward a **low-water** target: `high_water` decides when folding is needed, and `low_water` creates headroom so folding does not recur on nearly every subsequent turn. The fold never evicts to zero and never merely trims to just under the cap. See also: [Decision 15 in the short-term and medium-term memory design learnings](../impl/learnings/short-term-medium-term-memory-design.md#decision-15-lazy-read-watermarks-replace-trim-to-cap-thrash).
