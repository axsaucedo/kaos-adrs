...
Right-to-erasure is a declarative, scope-targeted hard delete that fans out **synchronously** across every tier that can contain the target's data: session-scoped conversational rows, medium-term summaries for those sessions, and Mem0 long-term facts filtered by the same trusted scope. Because [ADR 0004](./adr_0004_deployment-topology-and-control-plane.md) disables or ignores Mem0's local change-history copy, there is no KAOS-consumed shadow memory left behind. Erasure is synchronous and best-effort because it is infrequent and correctness matters more than latency. Deletion-verification ledgers and proof-of-deletion, and logical export and import, are deferred. See also: [Decision 1 and Decision 13 in the short-term and medium-term memory design learnings](../impl/learnings/short-term-medium-term-memory-design.md#decision-1-short-term-scope-is-session-only).
