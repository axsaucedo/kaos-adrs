...
- Grouping is expressed with one `scope` value and store membership, with no extra resource, keeping the first iteration simple while covering agent-private, per-user, and fleet-shared memory.
- A2A sub-agents are placed into scope by a server-injected, non-spoofable prefix that follows the delegation, with full-share and full-isolation overrides.
...
- Erasure removes scoped memory across both tiers in one synchronous operation with no lingering engine copy.
