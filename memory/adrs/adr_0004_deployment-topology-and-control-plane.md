...
The `MemoryStore` CRD describes **infrastructure, model bindings, and the two store-level operational knobs** — the extraction executor concurrency and the store-wide default failure mode. It deliberately does **not** carry the short-term-window, digest, or sweeper marks: those are memory-service configuration with sensible env defaults, because they govern a shared window the service owns and are not per-agent policy. Runtime behaviour that is genuinely agent-specific (the recall-window token budget and whether a rolling digest is kept) is overridden on the Agent memory block; everything else is a service default.
...
spec:
  ...
  extraction:
    concurrency: 4            # Mem0 long-term extraction executor pool
  defaultFailureMode: soft    # soft | strict; store-wide default for the write/forget path
```

The Agent CRD keeps the agent-specific memory policy and the two per-agent client parameters the runtime forwards, selecting a store by name. The `type` field states the backend intent explicitly — `remote` requires a bound `memoryStore` and uses the network `RemoteMemory` backend, `local` forbids one and uses the in-process short-term `LocalMemory` backend (recent-turn replay only, pod-local, no long-term tier) — and when omitted it is derived from `memoryStore` presence. A `type: local` block that also sets a `memoryStore` is rejected as a conflict. `RemoteMemory` is the renamed runtime HTTP client for this service — it speaks the KAOS tiered contract and is not a Mem0 client; Mem0 is embedded inside the service as the long-term engine, never called directly from the agent.
...
spec:
  config:
    memory:
      ...
      type: remote                     # local | remote; omitted => derived from memoryStore
      memoryStore: shared-memory       # required for remote; forbidden for local
      scope: user                      # private | user | shared | session
      clientParams:
        tokenBudget: 6000              # short-term recall window budget (per-agent)
        rollingSummary: true           # keep a medium-term rolling digest (per-agent)
      tools: all                       # all | read | write | (unset = none)
      failureMode: soft                # soft | strict; unset => inherit store default
```

`clientParams` is intentionally minimal: it exposes only the two knobs the runtime forwards per request — `token_budget` and `rolling_summary`. The shared-window, digest, sweeper, and extraction knobs (`compaction_trigger`, `compaction_target`, `hard_event_cap`, `digest_retention`, `idle_ttl`, `sweep_interval`, `extraction_concurrency`) are **service-side configuration**, not per-agent, because the short-term window is keyed by memory scope and shared across all agents bound to a store — a per-agent override of a shared-window mark is incoherent. Those knobs are memory-service env defaults with sensible values; the MemoryStore CRD surfaces only `extraction.concurrency` and `default_failure_mode` in this version, and promoting any window/digest mark to a CRD field is deferred until measurements justify it.
...
The operator validates the ADR 0003 window constraints (`compaction_target < compaction_trigger <= token_budget`, `hard_event_cap >= 1`, positive TTL/sweep intervals, positive digest budget when rolling summary is enabled, `concurrency >= 1`) at the level each knob is exposed: the per-agent `token_budget` on the Agent, and the service-side marks as memory-service defaults. The memory service is the executor of the policy because it owns the shared window, digest, locks, sweeper, and Mem0 executor.

The operator enforces the fail-closed binding guards that follow from the backend switch, expressing the same-object field relationships as CEL validation rules on the CRD and the cross-resource existence/readiness checks as reconcile-time conditions. `type: remote` requires a `memoryStore`; `type: local` forbids one. A `user`- or `shared`-scoped agent **requires** a `memoryStore`, because `LocalMemory` is pod-local and cannot serve cross-agent scopes — across replicas each pod would hold a divergent private copy. Setting `tools` also **requires** a `memoryStore`, because `save_memory` and `search_memory` target the long-term tier that `LocalMemory` lacks, so without a store they would be silent no-ops. All of these misconfigurations are rejected rather than silently degraded. A `private`-scoped owner is likewise resolved fail-closed from the agent's fully-qualified `kaos://agent/{namespace}/{name}` identity (injected as `AGENT_IDENTITY`), never a name-only or empty owner, so identity-less agents can never collapse onto one shared private partition. Store and model references resolve within the Agent's own namespace in this version; cross-namespace references are a governance concern deferred to [ADR 0005](./adr_0005_multi-tenancy-agent-grouping-and-governance.md).

When a bound `memoryStore` is not Ready, an **already-running** agent treats memory as a degraded augmentation rather than a hard dependency: the operator surfaces a `MemoryDegraded` condition on the Agent status and keeps the pod Ready so it continues serving short-term-only, instead of failing the pod readiness probe, marking the Agent not-Ready, or tearing down the Deployment — a store outage must never remove an already-serving agent. The one exception is the agent's **initial creation**: when `waitForDependencies` is set (the default) and the bound store is missing or not yet Ready, the operator gates the first Deployment (Agent `Waiting`, requeued) so the agent does not start up degraded, mirroring the ModelAPI/MCPServer dependency gate. Once the store turns Ready the MemoryStore watch requeues the agent and it is created; from then on a disappearing store degrades rather than gates.
...
- Once an agent is running, a memory outage degrades it to short-term-memory-only, surfaced through a `MemoryDegraded` Agent status condition, but never fails the pod readiness probe, tears down the Deployment, or halts serving; memory is an augmentation, not a tier-1 runtime dependency. Only the agent's initial creation is gated on the bound store being available (when `waitForDependencies` is set), so an agent never starts up degraded.
- The control plane splits the memory knobs by ownership: the per-agent recall budget and rolling-summary toggle on the Agent, the extraction concurrency and default failure mode on `MemoryStore`, and the shared-window/digest/sweeper marks as memory-service defaults — each validated where it is exposed.
- The Agent memory block is a single switch surface: enabling memory turns on the automatic recall-inject and write-extract baseline, an explicit `type: local|remote` (derived from `memoryStore` presence when omitted) selects the `RemoteMemory` service backend versus the pod-local short-term `LocalMemory` fallback, and a `tools: all|read|write` knob layers additive `save_memory`/`search_memory` tools; `type: remote`, `user`/`shared` scope, and any `tools` setting each require a bound store, `type: local` forbids one, and `private` ownership resolves fail-closed from the qualified agent identity.
...
- **All behavioural settings on the Agent.** Rejected: the memory service owns the shared window, sweeps, locks, digest retention, and executor capacity, so those are service-side defaults, and the two store-level operational knobs (`extraction.concurrency`, `defaultFailureMode`) belong on `MemoryStore`; only the genuinely per-agent recall budget and rolling-summary toggle remain on the Agent.
- **Full window/digest/sweeper knob set duplicated on both MemoryStore and Agent.** Rejected as incoherent and needless surface: the short-term window is keyed by memory scope and shared across all agents bound to a store, so a per-agent override of a shared-window mark has no well-defined meaning; the marks stay as memory-service defaults, promotable to CRD fields only if measurements justify it.
