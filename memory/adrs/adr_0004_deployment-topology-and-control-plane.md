...
When a bound `memoryStore` is not Ready, memory is treated as a degraded augmentation rather than a hard dependency: the operator surfaces a `MemoryDegraded` condition on the Agent status and keeps the pod Ready so it continues serving short-term-only, instead of failing the pod readiness probe or marking the Agent not-Ready — a store outage must never remove an agent from serving.
...
- A memory outage degrades agents to short-term-memory-only, surfaced through a `MemoryDegraded` Agent status condition, but never fails the pod readiness probe or halts serving; memory is an augmentation, not a tier-1 dependency.
