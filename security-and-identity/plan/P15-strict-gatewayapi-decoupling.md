...
- Add `--strict-gateway-api/--no-strict-gateway-api` (default off) mapping to `--set security.strictGatewayApi.enabled=true`. Update `--network-policy` help (drop the "Effective only with --auth-enabled" caveat, or remove the flag if fully superseded — decide and keep `--network-policy-egress` working).
