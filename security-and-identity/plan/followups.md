...
**Related — broker enduser `public_url` default (`iss` mismatch).** The broker defaults `server.enduser.public_url` to `http://localhost:8000` and stamps that value as the `iss` claim on the `client_credentials` agent tokens it mints. The gateway `agent` JWT provider validates those tokens against the broker's in-cluster issuer, so the mismatched `iss` makes every agent-identity request fail at the gateway with `Jwt_issuer_is_not_configured`. Unlike the ExtProc endpoint, the AIB chart *does* template this (`broker.server.enduser.publicUrl` → `public_url`), so KAOS now sets it to the in-cluster broker enduser URL at install time across all AIB postures (no upstream change needed). Remaining todo: fold this into the same `broker.extraEnv` cleanup so all broker self-URL/issuer settings are configured through one consistent mechanism, and add an install-time render assertion that `publicUrl` is always set to the projected agent-auth issuer.
...
**Promoted to phase P17** — the live-validation slice P16 could not run in-CI, plus the one feature gap it depends on. P16 collapsed the install surface into three presets and validated them at the unit/render/CI level, but two proofs remained environment-gated: (a) the `aib-keycloak` posture does not yet enable the broker `ext_proc` OPA authorization decision (`EXTPROC_AUTHORIZATION_*` + a mounted `granted_permission_sets` rego), so a valid actor token enforces JWT presence but not the grant graph; and (b) strict gateway-only enforcement is unproven because kindnet does not enforce NetworkPolicy (needs a Calico KIND cluster). See [`proposed-split.md`](./proposed-split.md) (P17) and the detailed plan [`P17-live-enforcement-validation.md`](./P17-live-enforcement-validation.md). The autonomous no-bearer gap ([F3](#f3--autonomous-mode-enforcement-g1-gap)) is explicitly out of P17's scope.

## F9 — Consent-based delegated access workflow (AIB's defining capability)

The KAOS↔AIB integration today wires only **agent identity** (broker-issued verified JWTs) and **authorization** (permission sets evaluated by the `ext_proc` OPA); the operator's AIB projector creates services, permission sets, and agents (permission sets bound as `requirement_type: "mandatory"`) plus per-agent credentials. It does **not** wire AIB's defining features: the **user-consent grant model**, the **consent UI** (human approves "agent A may access service S on my behalf"), or a **real third-party token vault** (the CLI's `third_party_service_id` is a `dummy-third-party` placeholder). Consequently the AIB capabilities KAOS currently uses (agent identity + OPA authz) overlap with what Keycloak + its OPA extension could provide; the parts that genuinely differentiate AIB (consent + vault) are unused.

This followup is to **design how KAOS wants to integrate the consent-based delegated-access workflow**: the user→agent→service consent data model, the human-in-the-loop consent screen, revocation, and a real vaulted third-party credential exchange gated on that consent — plus how it surfaces through KAOS CRDs, the operator projection, and the UI. It is the decision on whether (and how) KAOS commits to the delegated-third-party-access use case that justifies AIB over an IdP-only setup. Scoping/design only for now; larger than a single phase.
