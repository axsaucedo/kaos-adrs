...
**Scope** (KAOS CLI + chart + docs): collapse the ~26 fine-grained agent/user-auth, authorization, network-policy and TLS install flags into a single `--auth-enabled` preset selector with three curated postures — `aib-keycloak` (full Keycloak + identity broker + token exchange + verified OPA authorization; the default), `aib-only` (broker-issued verified agent identity without user login or token exchange), and `kaos-internal` (self-contained demo, KAOS-projected policy ConfigMap, header-trusted agent JWT, no external IdP/broker); keep `--gateway-api-strict` (from P15) and the infra flags visible and retain the granular flags as hidden advanced options so the E2E harness and power users are unaffected; author a security overview and align the authorization docs with the preset surface; validate both postures end-to-end.
