...
Introduce an application-level **request security context** and an **SDK that is not the enforcement boundary**. In the standard path the SDK carries verified identity/context across hops and keeps the agent actor token fresh so the gateway can enforce end-to-end ([ADR-KAOS-009](./ADR-KAOS-009-gateway-api-resource-boundary-enforcement.md)); it does **not** authenticate users and does **not** make authorization decisions — enforcement is gateway-centric. It additionally exposes optional access-check/token-exchange helpers for custom servers run off-gateway.
...
The actor is the calling agent's own identity, never the user token's `azp`. This is what makes multi-agent delegation correct ([ADR-KAOS-004](./ADR-KAOS-004-aib-responsibility-boundary.md)): when Agent B calls a resource, the gateway sees actor = B.
...
The instrumented HTTP clients attach the agent actor token and forward the user subject token transparently (including the machine-token refresh/retry described in [ADR-AIB-001](../adr-aib/ADR-AIB-001-aib-python-sdk-design.md)), so application code does not hand-pass auth pieces.
...
4. Managing the agent machine-token lifecycle (acquire/cache/refresh; see [ADR-AIB-001](../adr-aib/ADR-AIB-001-aib-python-sdk-design.md)).
...
Gateway remains useful as the optional resource-boundary enforcement layer; see [ADR-KAOS-002](./ADR-KAOS-002-enforcement-topology.md).
...
4. Use [ADR-KAOS-006](./ADR-KAOS-006-re-authentication-execution-model.md)'s fail-with-URL-and-retry behavior for consent and re-authentication, while leaving durable pause/resume to future task approval work.
