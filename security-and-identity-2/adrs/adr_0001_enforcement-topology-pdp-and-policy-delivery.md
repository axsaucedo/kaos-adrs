...
1. **KAOS-run PDP behind the gateway `ext_authz` seam (chosen)** — a KAOS-deployed decision service answering Envoy Gateway's `SecurityPolicy.spec.extAuth`, fed by the phase-1 identity plumbing and the projected ConfigMap. Inverts every research/001 blocker: ext_authz is a pure check with no exchange coupling, evaluates every request on the route (bearer or not — G1 closes by construction), and is fully KAOS-owned (policy content, data, upgrade cadence, enablement). Phase-1 ADR 0001 deliberately retained this seam as a default-off escape hatch; this option promotes it to the primary decision point. What implements the PDP is the next choice below.
...
4. **In-runtime enforcement (pais middleware)** — rejected in phase 1 as not a trust boundary (the workload enforcing on itself); remains valid only as defense-in-depth, not as the decision point.

### PDP implementation (given topology option 1)

- **Stock OPA image with the built-in `envoy_ext_authz_grpc` plugin (chosen)** — the minimise-changes option: zero KAOS service code to write, audit, or patch; the only KAOS-authored artifacts are the rego and the data document, and **both already exist from phase 1** (the static policy verifies tokens and checks `data.kaos.grants`; the projection produces the ConfigMap). Upstream CNCF project, standard pairing with Envoy ext_authz, and it natively supports the bring-your-own-rego mode the phase-1 pipeline already exposes.
- **A small KAOS-authored Go ext_authz server** — typed decisions, one fewer third-party image. Costs owning (writing, auditing, patching) a security-critical network server, and loses the bring-your-own-rego mode. Rejected for now; the published request-conventions contract keeps it swappable later.
- **Authorino (Kuadrant)** — an off-the-shelf ext_authz backend that can evaluate Keycloak Authorization Services and inline rego. The buy option if live IdP-side policy evaluation is ever wanted; today it would add a heavier component and a second policy-language surface for capabilities KAOS does not need.
...
- **What is deleted from phase 1**: the expectation that AIB's ext_proc authorizes anything, and any chart/config surface that existed solely to enable `#222`-style evaluation. Concretely, the chart's `authorization.provider` knob (`none`/`kaos`/`aib`) is deleted: `aib` is no longer an enforcement mode, `none` collapses into not deploying the PDP, and the sole remaining value needs no knob — `security.pdp.enabled` becomes the authorization switch (chart shape detailed in ADR 0002's appendix).
