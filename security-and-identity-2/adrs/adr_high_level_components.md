...
A subsequent cross-cutting design discussion converged the overall shape — three planes (identity / relationship / decision), CRDs as the relationship source of truth with the ConfigMap as compile target, IdP projection reduced to credential provisioning, preset-selectable agent issuers, and honestly scoped preset semantics — recorded in [research/005](../research/005-control-plane-pdp-and-relationship-model.md). The convergence also collapsed the original five ADRs into four: data delivery folded into the topology ADR (it is a property of the PDP deployment), the schema folded into the model ADR (the model and its data shape are one design), the AIB scope boundary folded into the identity ADR (AIB is now one optional issuer adapter), and the deferred token-exchange/consent track promoted to its own explicit-decision ADR.

## Components requiring an ADR

### ADR 0001 — Enforcement topology, the PDP, and policy delivery (supersedes phase-1 ADR 0001)

**Scope**: where the internal-traffic authorization decision runs (the PDP), which Envoy filter invokes it (the PEP), who owns the deciding component, and how policy and data reach it.

**Options to weigh** (topology):
...
**Key decisions** (leanings from the design discussion in parentheses):
- PDP deployment shape — converged on a separate lightweight Deployment rather than operator in-process/sidecar, so the control plane stays off the data path ([research/005](../research/005-control-plane-pdp-and-relationship-model.md)).
- Transport (converged: gRPC — `envoy.service.auth.v3.Authorization`, what OPA's envoy plugin speaks natively).
- Claim forwarding vs in-policy token verification (lean: keep in-policy verification against `data.kaos.jwks` with `jwt_authn` retained in front as the authn layer — both halves already exist from phase 1, and it makes forged claim headers cryptographically useless regardless of gateway misconfiguration; claim-forwarding recorded as the simplification path if measured latency ever demands it).
- Policy/data delivery and hot-reload (lean: ConfigMap volume mount now — zero new code, atomic symlink swap, but up to ~90s propagation for grants *and revocations* (F4); OPA bundle polling recorded as the evolution when revocation latency or per-replica load observability becomes a requirement). CRDs remain the source of truth; the ConfigMap is a *compile target* the operator regenerates, never a second store; PDP-watches-CRDs-directly (e.g. kube-mgmt) rejected — derivation logic would move into rego, the PDP would need K8s API permissions on the data path, and the manual modes plus atomic-snapshot property would be lost ([research/005](../research/005-control-plane-pdp-and-relationship-model.md)).
- Fail-closed semantics (`failOpen: false`; PDP unavailable → 403).
- Removal of AIB's ext_proc from the enforcement path, with filter-ordering consequences recorded for its possible ADR 0004 return.
- Publishing the KAOS request conventions (`x-agent-authorization`, `x-kaos-target-resource`, `kaos://` scheme) as the contract that makes any conforming ext_authz backend drop-in.
- Preservation of the automated / bring-your-own / operator-rego modes and the never-prune safety rule.

### ADR 0002 — Agent identity, issuers, and the AIB boundary

**Scope**: who issues agent (actor) credentials and tokens, how workloads obtain them, where actor tokens are verified, and the explicit contract of what KAOS uses AIB for.
...
- **OIDC provider clients provisioned via RFC 7591/7592 Dynamic Client Registration** — provider-generic `OIDCProjector` behind the existing `PolicyProjector` seam; Keycloak supports DCR natively. Converged default when agent identity must participate in the broader OAuth world (external callers, third-party APIs, multi-cluster, the ADR 0004 path).
- **AIB client_credentials + JWKS, excluding token exchange** — the proven phase-1 subset (registration, credential mint/rotate, LocalClient token minting, JWKS) via its admin API as a per-provider adapter behind the same seam; keeps the ADR 0004 consent path open.

**Key decisions**: issuers are preset-selectable behind one abstraction (converged: the gateway JWT provider and the policy see only an issuer, a JWKS, and a mapped identity); IdP projection reduced to credential provisioning only — client create/rotate/delete plus Secret delivery, with all authorization state out of the IdP; the AIB boundary — AIB is an *optional* issuer adapter, not a required component; **permission sets are exchange-scoped** (AIB's token-exchange-time concept, enabled in the adapter only with the ADR 0004 feature, never leaking into the PDP path); explicit re-entry criteria for deeper AIB integration — (a) AIB ships an authorization surface decoupled from exchange, or (b) KAOS adopts consent-based third-party delegation and AIB's consent/vault UX is the best implementation (assessed in ADR 0004); the `iss`/`public_url` consistency mechanism (F0); fate of the cancelled upstream-contribution and fork tracks; JWKS wiring for multi-issuer setups, including the K8s API server as issuer.

### ADR 0003 — Authorization model and data schema: user + agent + resource

**Scope**: the decision semantics — what facts constitute an authorization decision, how the user dimension enters it, and the `data.kaos.*` schema that carries the facts ([research/003](../research/003-user-agent-resource-authz-gap.md)).
...
**Key decisions**: the rego conjunction (subject-condition AND actor→resource grant); the autonomous "no subject" stance per posture (allow-on-agent-grants vs deny); extending the `data.kaos.*` schema with the user dimension without breaking the published `grants`/`jwks` contract; whether user facts are projected (from what source) or read live from token claims only; granularity (resource-level now, tool/route-level later — agentgateway demonstrates the tool-level dimension); behavior of `AccessGrant` when no user IdP is enabled (converged: accept, mark `Enforced=False` with `reason=NoUserIdentityProvider` status condition and warning event, skip projection — never silently ignore); a dedicated ReBAC store (SpiceDB/OpenFGA) recorded as the growth path only, not the starting point.

### ADR 0004 — AIB token exchange and the consent flow (explicit keep-in / defer decision)

**Scope**: the full, orthogonal AIB integration deferred so far as "F9" — delegated third-party access via RFC 8693 token exchange with user consent. This ADR exists to make the disposition an *explicit decision* rather than an indefinite deferral: design the integration end to end, price its complexity honestly, and decide in or out (and if out, under what re-entry conditions).

**Design questions the ADR must resolve**:
- **The consent flow** — where the consent screen lives and who serves it (AIB's UI, a KAOS-hosted page, or the IdP), how a user is redirected into consent mid-agent-interaction (agents call APIs, not browsers — the CIBA-style asynchronous approval problem), how consent state (`UserGrant`) is created, scoped, expired, and revoked, and what the UX is for a user managing standing consents.
- **Vault session establishment** — third-party OAuth sessions are only created via interactive authorization-code callbacks ([research/001](../research/001-why-aib-cannot-authorize-internal-traffic.md)); the ADR must design when and how users complete these flows (onboarding vs just-in-time) and what happens when a vaulted token expires mid-task.
- **Filter composition** — AIB's ext_proc re-enters as an *additional* filter alongside the ADR 0001 ext_authz PDP, scoped strictly to third-party egress routes; ordering, header ownership, and the guarantee that it never re-enters the internal enforcement path.
- **Chart and config surface** — the F0 gaps (env injection, `public_url`/`iss` consistency) that must be fixed upstream or worked around for a production deployment.
- **Complexity assessment and the decision** — total added components, state, and failure modes vs the concrete value of consent-based third-party delegation; the explicit outcome is one of: adopt now, defer with named re-entry triggers (a concrete third-party use case; AIB upstream maturity), or reject.

**Dependencies**: requires ADR 0002's AIB adapter (agents as AIB clients, permission-set binding) and composes with ADR 0001's topology; changes nothing in ADR 0003's model (the exchange governs token *acquisition* for third parties, not internal authorization — the separation AIB's own `#231` design principle endorses).
...
ADR 0001 (topology + PDP + delivery) unblocks everything and should be decided first. ADR 0002 (identity + AIB boundary) and ADR 0003 (model + schema) can proceed in parallel once 0001 lands, since they bind to the decision point it selects. ADR 0004 (exchange + consent) depends on 0002's AIB adapter and is deliberately last — its outcome (adopt / defer with triggers / reject) does not block the enforcement path.

Suggested order: **0001 → (0002 ∥ 0003) → 0004**.
