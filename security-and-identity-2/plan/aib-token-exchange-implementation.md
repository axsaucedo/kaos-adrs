...
**Status**: Proposed (for review; gated on a concrete third-party use case before execution)
**Realises**: [ADR 0004](../adrs/adr_0004_aib-token-exchange-and-consent.md) "The design (spike-validated)"
**Prerequisite**: a real third-party use case (e.g. an agent acting on GitHub as the user). Until then this stays deferred; the spike already proved viability.
...
## Phases (each = one plan-implement iteration = one PR; standard execution conventions apply)

### Phase E0 — Prerequisites (gate)
...
### Phase E1 — Declaration surface + AIB projection (operator, thin seam #2)
...
### Phase E2 — Runtime re-mint (`kaos_identity`)
...
### Phase E3 — Gateway ext_proc attachment (operator, thin seam #1)
...
### Phase E4 — Consent flow (fail → approve → retry)
...
### Phase E5 — Live end-to-end validation (extend the spike into a repeatable harness)
...
E0 (gate) → E1 (declaration + projection) ∥ E2 (runtime re-mint) → E3 (ext_proc attachment) → E4 (consent) → E5 (live validation) → E6 (docs). E1 and E2 are largely disjoint (operator/CRD vs runtime) and can parallelize on worktrees; E3–E4 depend on both.

## Known open decisions (resolve during E1/E3)
- Declaration surface shape: Agent spec field vs dedicated CRD.
- ext_proc route-attachment: per-route (operator, safe-by-construction — recommended) vs broad self-scoping (more disjoint, filter-logic-dependent).
- SA-primary exchange-credential provisioning (secondary Keycloak client) — or restrict exchange to `oidc`-identity agents.
